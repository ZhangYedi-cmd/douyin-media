...
大语言模型接入外部系统的探索经历过多轮演进。早期采用简单的函数调用（Function Calling）。随后出现了各类应用专属的插件体系。后来 Anthropic 发起并开源了 MCP（Model Context Protocol，模型上下文协议）。业界一直在尝试为大模型与外部工具之间建立一套通用标准。

标准化的初衷是降低系统集成的成本。在 MCP 出现前，每接入一个 SaaS 平台或本地数据库，开发者都需要在客户端手写一套工具描述、参数序列化与错误重试逻辑。MCP 试图把这件事标准化：服务端暴露出统一的工具、资源与提示词接口，客户端通过统一的传输协议完成握手与调用。

这个设想在理论上很完整。但在实际工程落地中，特别是在复杂的生产自动化流水线里，MCP 暴露出严重的上下文开销与稳定性瓶颈。
...
在标准 MCP 架构中，客户端（如 Claude Desktop、Cursor 或各类自研 Agent 框架）与服务端（MCP Server）建立连接。底层通信通道通常分为两类：基于本地子进程的标准输入输出（stdio），或者基于网络的 HTTP Server-Sent Events（SSE）。
...
上图展示了 MCP 的通信流程。左边是客户端与服务端在建立连接时获取全量工具定义，右边是客户端将所有工具定义注入提示词后发起模型推理与调用。
...
大语言模型本身没有操作系统进程的概念，无法主动探测本地磁盘有哪些二进制文件。大模型之所以能够发起工具调用，完全依赖客户端在每次请求中，将所有工具的 Schema 定义完整提交给模型。

这就产生了一个技术矛盾：MCP 的工具加载是静态全量常驻的，但实际业务调用往往集中在极少数工具上。

我们来测算一个典型的企业级 Agent 开发者环境。开发者通常会接入多个核心服务：
...
同时挂载这 5 个常用 MCP Server 时，静态常驻的工具描述就已经达到了 74 个，合计占用的静态上下文高达 8000 Token。

在单轮问答中，8000 Token 看起来成本有限。但在真实的工程任务中，Agent 需要经历思考、检索、编写代码、运行测试、修复报错等多轮往返迭代。一个中等复杂度的任务通常需要 20 到 30 轮推理。
...
在这一次任务中，仅仅为了让大模型知道这 74 个工具的存在，系统就传输了 20 万个 Token。而在这 74 个工具中，当前任务实际调用的通常只有两三个。

现代大模型 API 支持提示词缓存（Prompt Caching），静态工具定义的输入费用确实可以打折。

但提示词缓存机制有明确的适用边界：
第一，缓存只降低计费单价，不降低计算延迟。每次请求依然需要将这 8000 Token 的注意力权重完整加载到显存中参与计算。
第二，多轮对话中 tool_result 频繁追加导致缓存前缀匹配不稳定，一旦前置上下文发生动态变化，缓存极易击穿，退化为全额计费。
第三，缓存无法解决上下文窗口物理容量被挤占的问题。

当 8000 个 Token 被静态工具占据后，留给代码文件、错误堆栈与业务规则的有效记忆空间被直接压缩了接近一万 Token。这会导致长任务更容易发生遗忘与信息截断。
...
在 Transformer 的底层注意力机制中，模型生成每一个 Token 时，都需要在当前所有上下文的 Key 向量与当前 Query 向量之间计算相似度。当上下文中塞满了 70 多个工具的 Schema 描述时，注意力权重会被这海量的参数字段强行分散。
...
第三种是对系统核心业务规则的注意力丢失。
当 System Prompt 中工具描述的篇幅远远超过开发者编写的业务规范时，模型对业务约束的服从率会大幅下跌。开发者写在前面的代码必须写单测、发布前必须确认等核心纪律，很容易被后面数千行的 JSONSchema 淹没，导致模型在执行任务时直接跳过关键步骤。
...
静态全量常驻的机制，从物理规律上就限制了 Agent 接入工具的规模上限。要打破这个上限，必须引入按需加载机制。

---

## 2. 命令行界面的底层机制与架构优势

在计算机发展史上，命令行界面（CLI）已经稳定演进了半个世纪。从最初写 Unix 命令行，到今天在运维流水线里跑脚本，CLI 始终是软件工程中最通用的执行载体。
...
当大模型接收到与媒体资产相关的用户指令时，它会通过 Bash 工具在后台执行一次自省查询：

```bash
media --help
```

CLI 工具会在标准输出中打印出精简的帮助文本：
...
模型阅读了帮助信息后，参数格式已经完全清晰，随即执行真正的业务命令：
...
上图展示了 CLI 懒加载机制的时序流程。左边是系统启动时极低的初始上下文占用，中间是大模型根据实际需要动态查询参数格式，右边是基于确定性命令执行业务逻辑。
...
第二，文本密度高。
工程师编写命令行帮助信息时，措辞通常高度精练。一段 150 Token 的 `--help` 输出，其包含的功能说明和用法指引，往往比长达 1500 Token 的嵌套 JSONSchema 更加清晰易懂。

第三，分层按需探索。
大模型可以像工程师一样，先查一级命令，再查二级子命令。这种层级化的探索机制，从物理上隔绝了不同子命令之间的参数混淆，有效避免了工具过载引发的注意力稀释。
...
在工程架构中，状态机无需依赖模型的主观判断，只需要检查子进程的退出状态码是否为 0，即可做出确定的分支判断。
...
- 一个包含 30 步推理的长流程任务，累计产生的工具 Prompt 消耗为 15 万 Token。
...
两者在工具描述上的上下文消耗相差了百倍以上。省下来的上下文窗口，不仅大幅降低了 API 账单支出，更为模型容纳真实的业务日志和长代码片段留出了宝贵空间。

### 3.2 调试排错与本地单测体验对比

当生产环境出现偶发性工具调用失败时，排查步骤的长短决定了故障恢复的速度。

在 MCP 架构中，排查一个工具调用故障通常需要经历复杂的步骤：
1. 检查 Agent 客户端的配置文件，确认 MCP Server 启动参数是否正确。
2. 确认 stdio 进程是否存活，环境变量是否正确注入。
3. 检查客户端发出的 JSON-RPC 请求报文，确认 params 内部的字段是否被模型正确填充。
4. 查看 MCP Server 内部的控制台日志。

如果你在这一步报错了，多半是 MCP Server 内部抛了异常。异常直接写到了 stdout，把客户端的 JSON-RPC 解析器搞崩溃了。

而在 CLI 架构中，排查故障只需要工程师在终端重现那一行命令：
...
CLI 工具采用的是短生命周期模型（Fork-and-Exec）。
...
哪怕某一次命令因为极端输入发生了段错误或抛出未捕获异常，崩溃的也仅仅是那一个短暂的子进程。Agent 宿主进程毫发无损，只会收到一个非 0 退出码和 stderr 错误信息，并能在下一轮尝试中优雅恢复。

### 3.4 参数校验与状态安全性对比

安全性与状态隔离是生产环境不可妥协的底线。

在 MCP 架构中，安全控制通常依赖客户端层面的配置，例如配置某个工具是否需要人类手动批准。但这种批准粒度很粗。如果一个 MCP 工具名叫 `execute_sql`，弹出的确认框里往往只是一串深层嵌套的 JSON。审批人在几秒钟内很难肉眼看清这是只读查询还是删库命令。

在 CLI 架构中，安全控制可以下沉到操作系统与命令本身的双重防线：
1. 操作系统权限隔离：CLI 可以以受限的用户身份执行，通过 Linux 文件权限、容器只读挂载、SELinux 等底层机制杜绝越权写盘。
2. 命令行内置硬门禁：CLI 工具内部可以强制实行操作分级。例如高危命令必须显式传递 `--force` 或 `--dry-run` 标志，缺少标志直接报错拒绝执行。
3. 审计日志就地固化：每一次 CLI 命令的执行，都会被操作系统的 Shell 历史记录以及工具自身的审计日志模块原子性写入磁盘文件，审计追踪链条非常清晰。

---

## 4. 架构师决策模型：什么场景用 MCP？什么场景用 CLI？

选择工具形态需要根据具体业务场景的特征寻找最优解。
...
比如让 Agent 读取远程 Notion 知识库、检索 GitHub PR 讨论，或者查询 Linear 工单。这些场景由服务商官方维护标准 MCP 端点，开发者不需要自己写爬虫或 SDK 封装，拿来就能用。

第二，IDE 与桌面宿主的高内聚上下文注入。
在各类桌面端产品中，MCP 提供了 Resources 机制。宿主可以按需拉取文档切片，这与只读场景的契合度很高。
...
第一，任何引发系统状态变更的写操作。
包括数据库写操作、Git 代码提交与合入、自媒体内容发布、服务器配置修改、云资源创建与销毁。状态变更必须经过 CLI 层严格的参数校验、环境前置检查以及确定性退出状态码确认。

第二，重度文件 I/O 与长耗时流水线。
包括音视频转码渲染（如调用 ffmpeg）、全自动无头录屏、前端代码打包编译、批量数据清洗。这些任务耗时长、消耗系统资源大，CLI 能够在独立的子进程中稳定运行，并将实时进度刷入日志文件，避免打爆宿主连接。

第三，需要在现有自动化流水线里直接跑的系统工具。
有些能力既需要在本地终端手动执行，又需要在 GitHub Actions 里被自动化触发，同时还要让 AI Agent 能按需调用。这种情况下，CLI 是最通用的形态。
...
3. 调试困难：视频上传一旦因为网络超时卡住，MCP Server 的 stdio 进程就会一直阻塞。Agent 宿主直接失去响应，开发者在本地也很难单独拉起那个上传函数排查。
...
在实际构建系统时，不必把所有接口都往最新的协议上生搬硬套。

让 MCP 留在只读、跨生态的数据接入层，让 CLI 统领业务执行与状态流转。把工具做成可在终端单测的命令行，不仅让 AI 用得更稳定，也让维护系统的工程师睡得更踏实。

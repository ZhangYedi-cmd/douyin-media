...
质检靠判改分权，不靠自觉 — Side-by-side comparison

HEADLINE: 质检靠判改分权，不靠自觉
SUBHEAD: 判定权和修改权物理隔离

LEFT PANEL — 栏头胶囊：dubbing-reviewer
- 场景：戴帽质检员只拿放大镜和检查板，站在只读区，所有道具纯色无字
- 要点卡：Bash／Read／Grep／Glob／无 Edit／无 Write／只给 VERDICT 和改法

RIGHT PANEL — 栏头胶囊：主会话
- 场景：AI 机器人和黑框眼镜工程师在修复区改文案、重合成、再送检
- 要点卡：执行修复／重新合成／重新派审／最多 3 轮／第三轮仍 FAIL 交人

DIVIDER: 中间一条 #C8D0E4 竖线，中央白卡写「PASS／FAIL 报告」
FOOTER: 权限隔离是机制，不靠角色自觉
LABELS: Bash；Read；Grep；Glob；无 Edit；无 Write；VERDICT；PASS；FAIL；ROUND 1/3
ASPECT: 1536x1024

...
六项检查，一次过完 — Sequential checkpoints

HEADLINE: 六项检查，一次过完
SUBHEAD: 先验输入，再验产物；六项全过才 PASS

GATES (left → right, six compact gates with readable labels):
- Gate 1 标题：2.4 真相源同步｜清单：check-source-sync／audio-segments == narrations｜画面：质检员对照两张白卡
- Gate 2 标题：2.1 合成完整性｜清单：check-completeness／存在／非零／时长大于零｜画面：机器人点数音频块
- Gate 3 标题：2.2 语速离群｜清单：check-pace／过快／过慢／统计假象不 FAIL｜画面：工程师查看速度表
- Gate 4 标题：2.3 死气停顿｜清单：check-silence／大于 0.45 秒｜画面：紫怪兽藏在声波空白处
- Gate 5 标题：1.2 多音字念法｜清单：check-pronunciation／按语境裁决｜画面：工程师戴耳机核语境
- Gate 6 标题：3.1 音画一致｜清单：check-av-consistency／组件大字对 narration｜画面：质检员对照画面和字幕
PASSING ARTIFACT: 一整批配音 mp3
AXIS: 输入正确 → 产物完整 → 综合判断
FOOTER BAR: 六项全过，可进录屏
LABELS: 2.4；2.1；2.2；2.3；1.2；3.1；check-source-sync；check-completeness；check-pace；check-silence；check-pronunciation；check-av-consistency
ASPECT: 1536x1024

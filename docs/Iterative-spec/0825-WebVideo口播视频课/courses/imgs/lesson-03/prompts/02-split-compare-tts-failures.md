...
TTS 的两类典型病 — Side-by-side comparison

HEADLINE: TTS 的两类典型病
SUBHEAD: 多音字靠语境判，死气靠数字扫

LEFT PANEL — 栏头胶囊：多音字
- 场景：工程师戴耳机对照三张白色语境卡，AI 机器人等待修订指令
- 要点卡：各十几行／就得打折／诚实得多／按段 overrides／同段两读只能改文案

RIGHT PANEL — 栏头胶囊：死气
- 场景：质检员用放大镜检查声波中一段过长空白，紫色怪兽躲在静音区
- 要点卡：超过 0.45 秒／depause／先写 .tmp.mp3／mv 回原文件／复扫归零

DIVIDER: 中间一条 #C8D0E4 竖线，中央一个 VS 徽标
FOOTER: 两类问题都不能只凭听感，必须留下可验输出
LABELS: 各十几行；就得打折；诚实得多；overrides；0.45 秒；depause；.tmp.mp3；复扫归零
ASPECT: 1536x1024

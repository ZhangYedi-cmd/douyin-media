...
改文案后的强制重生成纪律 — Sequential checkpoints

HEADLINE: 改文案后的强制重生成纪律
SUBHEAD: 三步制作纪律，加一道最终同步检查

GATES (left → right):
- Gate 1 标题：重新抽取｜清单：npm run extract-narrations／更新 audio-segments.json｜画面：AI 机器人从 narrations.ts 重新生成清单
- Gate 2 标题：删除旧音频｜清单：rm public/audio/<chapter>/<step>.mp3／防止 skip｜画面：工程师移走旧音频；垃圾桶中的文件页、文件夹和音频盒全部使用纯色无字表面，不得印 MP3、图标、音符或任何其他字符；紫怪兽试图藏住旧文件
- Gate 3 标题：重新合成｜清单：synthesize.mjs／改动段 mp3｜画面：机器人生成新的声波文件
- Gate 4 标题：源同步检查｜清单：check-source-sync.mjs／全部 N 段一致｜画面：质检员举绿牌，对照两份文本
PASSING ARTIFACT: 改过的口播文案
AXIS: 少一步，就可能音画不符
FOOTER BAR: 旧文件不删，合成器会 skip 并装作干完
LABELS: npm run extract-narrations；audio-segments.json；rm public/audio/<chapter>/<step>.mp3；synthesize.mjs；check-source-sync.mjs；全部 N 段一致
ASPECT: 1536x1024
...
HARD CONSTRAINTS:
- In Gate 2, every discarded audio prop is visually blank: no "MP3", no music-note icon, no filename, no symbol, and no invented caption on papers, folders, boxes, bin contents, or objects held by the engineer or monster. The only visible `mp3` text belongs in the named 清单 / FOOTER / LABELS fields.

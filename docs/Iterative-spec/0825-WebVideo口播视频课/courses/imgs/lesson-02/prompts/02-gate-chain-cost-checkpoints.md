...
停顿放在返工成本突变处 — Sequential checkpoints

HEADLINE: 停顿放在返工成本突变处
SUBHEAD: 介入次数少，但每次都挡在批量返工之前

GATES (left → right, each stricter and more expensive):
- Gate 1 标题：Checkpoint Plan｜清单：script.md／outline.md／五件事对齐｜画面：工程师在两张文件卡前盖确认章
- Gate 2 标题：第 1 章验收｜清单：每次点击一拍／信息来自原文／视觉主题一致｜画面：质检员逐屏检查第 1 章
- Gate 3 标题：Checkpoint Audio｜清单：网页骨架完成／口播分段确认／进入配音｜画面：AI 机器人停在音频闸门前等待人确认
PASSING ARTIFACT: article.md 逐步变成完整可点击网页
AXIS: 越往右，返工成本越高
FOOTER BAR: 一次对齐挡掉后面十次返工
LABELS: Checkpoint Plan；script.md；outline.md；第 1 章验收；Checkpoint Audio
ASPECT: 1536x1024

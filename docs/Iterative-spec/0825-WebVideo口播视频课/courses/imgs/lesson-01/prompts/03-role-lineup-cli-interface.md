...
CLI 是三方通吃的接口 — Role lineup

HEADLINE: CLI 是三方通吃的接口
SUBHEAD: 同一份文本契约，同时服务 LLM、机器和人

CHARACTERS (3 个并排站立，姿态各异):
- AI 机器人｜标题：对 LLM｜副标题：文本是母语｜清单：文本进／文本出／check --json｜画面：机器人阅读一张纯白无字卡，卡上不画任何 JSON 字段、代码、数字或标点
- 自动化质检员｜标题：对机器｜副标题：可组合、可定时｜清单：脚本／cron／日志｜画面：质检员把三个齿轮节点连成自动链
- 黑框眼镜工程师｜标题：对人｜副标题：可复制、可追责｜清单：粘贴命令／查看输出／原样求助｜画面：工程师在纯图形化的空白终端旁核对命令和输出，屏幕上不画任何代码、英文、数字或文字
TOP LABELS: LLM／机器／人
BOTTOM BAR: CLI 为组合设计，接口就是契约
LABELS: 文本进；文本出；check --json；脚本；cron；日志；命令和输出
ASPECT: 1536x1024

RETRY-SPECIFIC HARD CONSTRAINTS:
- The white card and terminal screen are illustrations only and contain ZERO visible text, code, JSON keys, commands, metrics, or numbers. Do not invent examples such as action, target, cli run test, status, passed, or failed.
- Render visible technical text only in the named list cards: 文本进／文本出／check --json／脚本／cron／日志／粘贴命令／查看输出／原样求助. No other code-like or metric-like text may appear.
- Show all three characters as complete full-body figures, including legs and feet, above or beside their cards. No card, screen, divider, or canvas edge may crop or hide their lower bodies.

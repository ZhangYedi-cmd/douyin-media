...
约束的生命周期 — Layered hierarchy

HEADLINE: 约束的生命周期
SUBHEAD: 从一次对话，到一类任务，再到整个项目

LAYERS (bottom → top):
1. 标题：你的消息｜副标题：只管这一次对话｜清单：意图 + 约束｜画面：工程师递给机器人一张临时任务卡
2. 标题：SKILL.md｜副标题：管一类任务怎么做｜清单：方法论／按需加载／可复用｜画面：机器人从说明书架取下一张流程卡
3. 标题：CLAUDE.md｜副标题：管整个项目永远遵守什么｜清单：项目宪法／自动加载／进 git 审查｜画面：质检员守在项目入口，规则悬挂在白色平面卡上
LEFT AXIS: 越往上，生命周期越长
RIGHT AXIS: 越往上，复用范围越大
FOOTER PILL: 对话会结束，文件才是人和 AI 的共享记忆
LABELS: 你的消息；意图 + 约束；SKILL.md；方法论；CLAUDE.md；项目宪法
ASPECT: 1536x1024

...
要想让自动化系统能够自己修好报错，关键是把生成和审查拆开，让每一步状态流转都有据可查。
...
划分这三个出口的目的，是杜绝中间模糊状态。自动化程序要么带着明确的通过凭证向下游移交，要么带着完整的错误快照停下等待人工干预，不在未经验证的情况下把残次品推入后续环节。
...
语音合成引擎在遇到中文引号、破折号或括号时，会在前后机械插入近 1 秒的无声等待。这种停顿直接破坏短视频的紧凑节奏，导致完播率大幅下滑。

自愈程序通过调用 depause.mjs 脚本切除静音。脚本内部利用 ffmpeg 的 silencedetect 滤镜捕获超过 0.45 秒的静音起止点。接着通过 atrim 与 concat 滤镜，把长静音压缩至 0.25 秒的自然呼吸气口。
...
如果在修复过程中删除了中间段落（例如 step_03），后续段落编号会整体前移一位。如果 overrides 仍沿用旧编号，给第 4 段配置的注音就会误注入给第 3 段，造成旧段落失效与新段落错乱。

因此自愈程序在执行配置注入前，必须先重新运行提取脚本同步段落索引，再对 overrides 里的所有键名进行重新对齐与清理。

如果你删除了某个段落后，发现后续段落发音全部读错，多半是 build/tts.config.json 里的 overrides 键名还保留着旧编号。增删段落后必须先重排索引，然后再注入发音配置。
...
在前半句中，得读作 děi，表示必须；在后半句中，得读作轻声 de，作为补语助词。市面上绝大多数语音合成接口的注音替换字典是基于文本子串进行全词替换的。如果在当前段落的配置里把得注音为 děi，后半句就会被合成出异常的诚实得（děi）多；如果注音为 de，前半句又会变成系统得（de）打折。
...
文案重构触发之后，必须严格执行多源同步纪律。在前端动态演示视频架构中，口播文案同时存在于三个地方：口播脚本源文件 2-script.md、前端解说契约文件 narrations.ts 以及具体的渲染组件。
...
这通常有三种原因。第一，语音合成引擎没有收录目标专业词汇的正确声调。第二，提示词对文案字数与排版空间提出了互相冲突的要求。第三，大模型在长上下文下陷入了固执的推理幻觉。
...
在生产环境中，视频录制通常通过无头 Chromium 浏览器以自动化脚本驱动。无头浏览器在后台运行时，存在两个隐蔽的陷阱。

第一个陷阱是无头渲染时间膨胀。当演示页面中包含大量的 CSS 动画、Canvas 粒子或高频 DOM 重绘时，无头浏览器的实际渲染帧率可能会低于每秒 60 帧。原本在物理时间中应该耗时 1 秒播放完的动画，在无头环境里被拉长到了 1.3 秒。然而音频切片是以恒定物理速率播放的，这直接导致录出来的视频在后半段出现画面严重落后于声音的脱节现象。

第二个陷阱是参数覆盖导致的字幕漏烧。在录制脚本 auto-record.mjs 中，默认访问地址往往不带字幕参数。如果启动录屏服务时带了 --serve 参数，脚本默认路由会把原本带有 ?subs=1 的链接覆盖掉，最终录出没有任何字幕的裸视频。
...
如果视频轨道的总时长超过音频轨道总时长的 5% 以上，说明在录制过程中发生了严重的无头渲染膨胀。此时自愈程序会自动使用 ffmpeg 的 setpts 滤镜对视频轨执行时间轴压实重混流，或者降低录制分辨率后重新执行录屏。
...
### 5.1 第一步：自愈回路启动提示词（测试日志信封化）

当 dubbing-reviewer 首次返回 FAIL 时，主控调度器将结构化判决结果与上下文组装为第一步自愈启动信封，投喂给修补 Agent。

````markdown
# 角色定义
你是一个专门负责修复音频合成与口播契约缺陷的工程修补 Agent。你的任务是根据客观测试日志，消除报错，使系统通过下一轮质检。
...
````markdown
# 角色定义
你是一个专门负责处理残留缺陷的自愈修补 Agent。系统在前一轮修复后仍未完全通过质检，你需要结合历史轨迹分析根因，完成收敛修复。

# 当前任务上下文
- 任务目录：<TARGET_DIR>
- 当前自愈轮次：Round <CURRENT_ROUND> / Max 3（达到第 3 轮未通过将直接触发熔断挂起）
...
通过物理度量工具建立客观质检门禁，把模糊的语音和画面转化为精准的结构化信封。通过职责解耦实现生成者与审查者的双向博弈。通过清晰的缺陷路由实现常见问题的自动化修复。通过死死定住的 3 轮硬熔断为整个系统兜底止损。这四套机制组合在一起，流水线才真正具备了在无人值守状态下持续稳定运转的工程底气。

反正能跑通的系统，骨子里都是这套克制的逻辑。

### 课后实战作业

1. 多音字自动注音自愈实战：在测试目录中编写包含行业术语各十几行的文案（默认易读错为 xíng）。编写调度脚本，将测试失败日志打包投喂给自愈提示词。验证系统能否自动把拼音映射写入 build/tts.config.json 的 overrides 字典，并重新合成出正确音频。
2. 同段两读死锁熔断实战：在文案中植入包含就得与诚实得多的同段两读冲突句式。启动带 3 轮熔断限制的自动化调度流程，观察前两轮的修补尝试。验证系统在第 3 轮未收敛时，能否准确将状态翻转为 drafting_blocked，并输出人工接管工单。

...
这四步实际怎么穿插执行，尤其是录制命令为什么要先配好、等配音合成完再回头跑，看这张图。

```mermaid
flowchart TD
    A["写口播稿 2-script.md"] --> B[web-video-presentation 建网页工程]
    B --> C[加字幕层]
    C --> D[抽取 narrations 生成分段文案]
    D --> E[造 tts-dub skill 配音合成]
    E --> F[回头执行录制命令出片]
    F --> G[抽帧校验]
    G --> H[baoyu-image-gen 出封面]
    H --> I[产物归位]
```

...
echo "▸ 使用主题：$THEME"
# npm create vite 会把绝对路径开头的 "/" 吃掉（按相对项目名 sanitize），
# 故 cd 到父目录、只把 basename 传给它，绝对/相对路径都能在正确位置创建。
# npm_config_yes 让首次 npx 拉 create-vite 时不弹确认（无人环境必须）。
_parent="$(dirname "$TARGET")"; _base="$(basename "$TARGET")"
mkdir -p "$_parent"
( cd "$_parent" && npm_config_yes=true npm create vite@latest "$_base" -- --template react-ts >/dev/null )
...
echo "▸ 安装 playwright + chromium（用于无人录制 npm run record）..."
...
cp "$TEMPLATES/scripts/auto-record.mjs"        scripts/auto-record.mjs
...
p.scripts = Object.assign({}, p.scripts, {
  ...
  "record":             "node scripts/auto-record.mjs",
});
...
录制：
  ...
  • 无人出片（推荐，流水线用）：
    ...
    headless 起服务、按音频时长确定性推进、录画面、ffmpeg 配音 mux → mp4，
    全程零点击。要求每个 step 都有口播（无静音步）。详见 references/RECORDING.md。
  • 人工预览三模式（调试用）：
      手动 http://localhost:5174（点击 / 方向键推进）；
      ?audio=1 音频跟 step 切手动推进；?auto=1 按 SPACE 自动播放，M 键切换。

...
> 它本身**不干治理活、不碰 brain/线上**——只决定"跑谁",活和硬约束(只产报告、人审后应用)在各执行 skill 里。

## 开工前必读
1. `harness/README.md`(治理线宪法 + 硬约束)。
...
5. **记账**:每个 (任务,目标) append 一行到 `logs/index.jsonl`:
   ...
   ```
   `applied:false` = 仅出报告待人审(治理硬约束);人审应用后由人/审计把它翻 true。
6. **汇报**:跟用户说这轮跑了哪些任务、产了哪些报告待审、哪些没到点跳过。**不替人审、不应用变更**。

## 红线 / 边界
- **只调度,不治理**:不直接改 `brain/`、不改线上、不替执行 skill 做事。

...
- 当前启用 4 个：养大脑的 `retro`（复盘，阶段契约另见 `retro.md`）、`benchmark-refresher`；养池子的 `ideate`（2026-07-08 自生产线迁入）、`backlog-gardener`。
- 另 6 个 TODO 登记在册（disabled，含元层 account-audit），**任务卡即接入规格**：接入 = 按卡写执行 skill + 翻 `enabled`。

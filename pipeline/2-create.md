...
6. **封面生成（★ 必做 · 竖版 9:16 · 交审前强制，不可跳过）**：用 **`baoyu-image-gen`** 出竖屏封面 → `assets/cover.png`（**禁止拿横屏视频帧充数**，L1）。
   - **风格选择**：
     - 用户**指定了**封面视觉风格 → 按用户的来。
     - 用户**没指定** → 用 baoyu-image-gen 生成「最合适」的；**本系列（claude-code-source-series）默认 = 仿 EP01 系列统一风格**：以 EP01 `cover.png` 当 `--ref` 锁定身份，**复用那只方块像素吉祥物本体**（别让模型新编角色，L1），暗底+虚化代码+单热橙+顶部胶囊徽章「Claude Code 源码解读·EPxx」+底部大字双行标题（橙高亮）。中心姿态/标题按本集内容定（"最合适"体现在这）。
...
> ★ 教训 L4：分轮反应式修 = 反复重渲染。**所有检查一次性前置过完再交审**；组件截图只证明画面，不证明声音/节奏/成片。
...
**G. 发布物料 `4-publish.md`**（★ 出审前必产，L5/L6）
...
> 踩坑故事与全量教训见 `pipeline/lessons.md`（本文规则标 Lx 的都能在那查到出处）。

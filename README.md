...
- **工作日**：定时 agent 自动跑 `选题 → 创作 → 出审`（见 `pipeline/daily-run.md`），停在人审环节。
...
| 路径 | 作用 |
|---|---|
| `brain/` | 账号大脑：定位/人设/风格/对标/信息源 + 账号级配置（tts.config.json） |
| `pipeline/` | 生产线自包含：各阶段 SOP + 编排入口 `daily-run.md` + 运行日志 `logs/` |
| `harness/` | 治理线自包含：宪法/任务注册表/复盘 SOP/治理日志与数据 |
| `content/` | 内容条目：每条内容一个目录，从选题到复盘全在里面 |
| `tools/console/` | 集成式控制台：核心领域能力、服务端、CLI 与 Web UI |

...
在纯命令行环境下，为了查看流水线状态，通常需要手动执行多条查看命令，或者在文件管理器里一层层翻找各个目录下的元数据。这种黑盒状态不仅增加了日常巡检的心智负担，也使得多任务协作变得迟钝。
...
解决这个问题的方案并不需要引入庞大复杂的前后端工程。搭建一套基于服务端事件流（Server-Sent Events，SSE）的轻量级本地控制台，能够在零外部数据库依赖、零繁重构建配置的前提下，实现文件状态变动到浏览器界面的秒级实时同步。

## 1. 技术选型：为什么是原生 Node.js 与 SSE？

在决定为自动化系统开发可视化界面时，很多开发者的第一反应是搬出企业级管理系统的技术栈：前端起一个基于 Vite 的 React 或 Vue 工程，后端用 NestJS 或 SpringBoot，中间挂一个 PostgreSQL 数据库存放状态，再配置 Redis 消息队列、WebSocket 双向通道和 Nginx 反向代理。
...
第三是资源开销过大。一个基于 Node.js 原生模块的轻量服务运行时代价通常在 30MB 内存以内，而包含 Docker 容器、数据库引擎、Java 虚拟机或多套构建工具的后台体系，会长期占用数个 G 的内存和可观的 CPU 算力，挤占视频渲染和音频合成等核心生产环节的计算资源。
...
WebSocket 提供了全双工的双向通信能力。但建立 WebSocket 需要专门的协议握手、心跳维持、连接保活和复杂的状态机管理。在 Node.js 中往往需要引入额外的第三方依赖库。最重要的是，控制台的前端对后端的需求绝大多数时候是单向的数据广播，并不需要长连接上的高频上行通信。
...
SSE 在本系统中的核心定位不是传输庞大的数据包，而是作为通知信号枪。
...
第二步的目标是构建纯原生前端单页应用。要求不依赖 Webpack、Vite、React 或 Vue，仅靠原生 HTML、CSS 和 JavaScript 实现多 Tab 切换、数据渲染与 SSE 事件监听。
...
看板的核心功能包括：
1. 分数矩阵列表：默认按综合传播力评分（Score）从高到低排序，展示选题的标题、核心卖点、来源平台（X / Reddit / GitHub / Hacker News）以及所属赛道（源码解读 / AI 实战 / 行业新闻）。
...
看板分为上下两个板块：
- 上半部分展示治理任务注册表（Tasks Registry），列出每个治理任务（如复盘分析 `douyin-retro`、对标账号复核 `benchmark-refresher`、选题池理池 `backlog-gardener`）的触发周期、上次执行时间、当前状态以及运行成功率。

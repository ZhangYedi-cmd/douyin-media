...
**[2]** 说白了，就是写代码前先让AI把「要干什么」写成说明书，人和AI都照这份说明书来，不是靠一句话让AI自己脑补。今天我把几家的做法扒了一遍，顺便告诉你这事到底成不成熟。

## 第二章 · 三个认知点（build，10 段）

**[3]** 先说为什么需要它。AI写代码有两个老毛病：第一，过度自信——拿到一句简短需求就敢动手，写错了还嘴硬说修好了，遇到冷门开源库，容易凭感觉编一个不存在的函数。
...
**[6]** 现在做这件事的主要是四家。第一个，GitHub Spec Kit，微软背后推的，流程最完整——先立「宪法」定原则，再写需求、消歧义、出方案、拆任务、最后实现，一套走完能生成七八份文档，是这里面最规范也最重的一个。

**[7]** 第二个，亚马逊Kiro，直接做成IDE产品，只用三步：需求、设计、任务。最大亮点是「hooks」——存文件、提交代码这些动作能自动触发检查，比如自动扫有没有漏了密钥。

**[8]** 第三个，Tessl，走得最激进，想让说明书直接就是代码——人只改说明书，代码顶部写着「生成的，别手动改」。它还做了个说明书库，收了一万多条开源库的精确用法，专治AI瞎编API。

**[9]** 第四个，OpenSpec，开源轻量，专盯存量项目——前面几家教程大多从零建新项目，但真实工作大部分是在改老代码，这个刚好补上这块空。
...
**[12]** 也有实打实的效果：微软自己的案例，把「新增一种资产类型」这种重复的活提炼成模板化说明书后，交付周期从两三周压缩到几天——不过这是厂商自己的案例，效果因团队而异，别直接照搬预期。

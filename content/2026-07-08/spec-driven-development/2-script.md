...
**[1]** 你有没有遇到过这种事：AI写错了代码，你说它错了，它立刻回你「你说得对，我马上修」，然后压根没修对。这半年，GitHub、亚马逊、一堆AI公司不约而同在推同一个词：Spec Coding，想解决的就是这个尴尬。

**[2]** 说白了，就是写代码前先让AI把「要干什么」写成一份说明书，人和AI都照着这份说明书来，不是靠一句话让AI自己脑补。今天我把这几家的做法扒了一遍，顺便告诉你，这事到底成不成熟。

## 第二章 · 三个认知点（build，10 段）

**[3]** 先说为什么需要它。AI写代码有两个老毛病：第一，过度自信。拿到一句简短的需求就敢动手，写错了还嘴硬说修好了，遇到不常见的开源库，容易凭感觉编一个根本不存在的函数出来。

**[4]** 第二，代码库越大，AI为了完成你交代的这一个任务，越可能悄悄把别的功能改坏——行话叫「reward-seeking」，说白了就是「先把眼前的活干完，管它牵连了啥」。

**[5]** 还有个更本质的问题：你的需求从脑子里到最后变成代码，中间要经过好几次转手——先变产品需求，再变技术方案，再变代码。微软今年6月的一篇博客说得很直白：每次转手都可能走样，靠聊天记录传需求，复杂度一高必然对不上。Spec，就是想把这份说明书显性地、完整地留下来，变成人和AI共同的真相来源。

**[6]** 现在做这件事的，主要是四家。第一个，GitHub Spec Kit，微软背后推的，流程最完整——先立「宪法」定死原则，再写需求、消歧义、出技术方案、拆任务、最后实现，一套走完能生成七八份文档，是这里面最规范、也最重的一个。

**[7]** 第二个，亚马逊的Kiro，直接做成一个IDE产品，只用三步：需求、设计、任务。最大的亮点是「hooks」——你存文件、提交代码这些动作，能自动触发检查，比如自动扫有没有漏了密钥。

**[8]** 第三个，Tessl，走得最激进，想让说明书直接就是代码本身——人只改说明书，代码顶部写着「这是生成的，别手动改」。它还顺手做了个说明书库，收了一万多条开源库的精确用法，专治AI瞎编API。

**[9]** 第四个，OpenSpec，开源、轻量，专盯存量项目——前面几家的教程大多是从零建一个新项目，但真实工作大部分时候是在改老代码，这个刚好补上这块空。

**[10]** 但我得说句实话，这些工具现在都还挺糙。有人拿一个很小的bug去测Kiro，结果被拆成4个用户故事、16条验收标准——用大锤砸核桃，小问题反而办得更慢。

**[11]** 用Spec Kit测一个中等功能，产出七八份markdown要你审，他说了句大实话：我宁愿直接看代码。而且就算写了这么多规则，AI照样会挑着不听——上下文窗口变大，不代表它真把里面的东西都当回事。

**[12]** 当然也有实打实的效果：微软自己的一个案例，把「新增一种资产类型」这种重复出现的活提炼成一份模板化说明书之后，交付周期从两三周压缩到了几天——不过这是厂商自己发的案例，效果因团队而异，别直接照搬预期。

## 第三章 · 结论与收官（ending，2 段）

**[13]** 所以结论是什么？这个理念是对的——写代码前先把「要干什么」讲清楚，这件事永远有价值。但工具层面还早期，别一上来就套最重的那一套，先从「把需求写清楚再动手」这个习惯开始就够了。

**[14]** 你现在用AI编程，是直接一句话甩过去，还是会先写清楚需求？评论区告诉我。
...
- **合规**：无违禁/绝对化承诺（"唯一"用于客观技术差异化描述，非营销自夸）；互动引导为评论区提问，非站外导流；未编造任何数字/benchmark，[12]数据已标注来源方与适用限制。
